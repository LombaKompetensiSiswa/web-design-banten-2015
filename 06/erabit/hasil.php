...
$colname_barang = "-1";
if (isset($_GET['cari'])) {
  $colname_barang = $_GET['cari'];
}
mysql_select_db($database_koneksi, $koneksi);
$query_barang = sprintf("SELECT * FROM produk WHERE namabarang LIKE %s", GetSQLValueString("%" . $colname_barang . "%", "text"));
...
<div id="konten">
  <div id="rightbar">
    <h1>Hasil Pencarian : <?php echo $_GET['cari'] ?></h1>
    <h2>
      <?php if ($totalRows_barang == 0) { // Show if recordset empty ?>
      </MM:DECORATION></MM_HIDDENREGION>
    </h2>
    <MM_HIDDENREGION><MM:DECORATION OUTLINE="Show%20If..." OUTLINEID=4>
      <h2>Tidak ada hasil untuk <?php echo $_GET['cari'] ?></h2>
      <h2>
        <?php } // Show if recordset empty ?>
    </h2>
    <?php do { ?>
        <?php if ($totalRows_barang > 0) { // Show if recordset not empty ?>
  <div class="produk">
